...
void OldNameMapper::addReplacement( std::string variableName,
    ...
    baseReplacementVec.push_back( ReplacementInfo( variableName, replacement, baseReplacementVec.size() + 1 ) );
}

void OldNameMapper::addReplacement( std::string variableName, std::string replacement){
    addReplacement( variableName, replacement, m_generalReplacements );
}

void OldNameMapper::addReplacement( std::string partName,
                                    std::string variableName,
                                    std::string replacement){
    // check if vec for particle type exists yet
    //~ std::vector< ReplacementInfo > &partVec = std::vector< ReplacementInfo > ();
    ...
    addReplacement( variableName, replacement,  m_particleReplacements[partName] );
}

std::string OldNameMapper::getUserRecordName(const pxl::Particle* &obj, std::string partName, std::string variableName )const{
    const pxl::Object *ptr = obj;
    return const_cast<OldNameMapper*>(this)->getUserRecordName( ptr, partName, variableName );
}
std::string OldNameMapper::getUserRecordName(const pxl::Object* &obj, std::string partName, std::string variableName ){
    ...
    return getUserRecordName( obj, variableName, basePartMap, cachePartMap );
}

std::string OldNameMapper::getUserRecordName( const pxl::Object* &obj, std::string variableName){
    return  getUserRecordName( obj, variableName, m_generalReplacements, m_ActiveGeneralReplacements );
}
