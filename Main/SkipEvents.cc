...
bool SkipEvents::skip( unsigned int const runNumber,
   ...
   if( runNumber == m_dontSkip.first ) {
      ...
      if( lumiSection == m_dontSkip.second ) return false;
   }
   ...
   if( run == m_skipRunsLumisEvents.end() ) {
      ...
      if( (*run).second.empty() ) return true;
